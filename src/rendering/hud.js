...
export function renderHUD(world) {
  ...
  right.innerHTML = `
    <span class="badge">HP: ${Math.ceil(world.player.hp)}/${world.player.maxHp} (${hpPct.toFixed(
      0
    )}%)</span>
  ...
  left2.innerHTML = `
    ...
    <span class="badge perm">+Range%: ${(perm.rangePct * 100).toFixed(
      1
    )}</span>
    <span class="badge perm">+Speed%: ${(perm.speedPct * 100).toFixed(
      1
    )}</span>
